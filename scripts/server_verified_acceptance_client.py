...
single_result_new="""          wo._edgeResult = _result;
          ...
          const _edgeCount = Number(_result.count || _result.signatureCount || 0);"""
if s.count(single_result_old)!=1: raise SystemExit('single result marker missing/ambiguous')
...
bulk_result_new="""  window._bulkWo._edgeResult=edgeResult;
  ...
  renderBulkResultModal(orders,edgeResult,file.name,b64);"""
if s.count(bulk_result_old)!=1: raise SystemExit('bulk result marker missing/ambiguous')
...
required=[
  "workOrderIds:[wo.id]",
  "signedPdfPath:wo.signedPdfPath||''",
  "workOrderIds:orders.map(w=>w.id)",
  "rodios_finalize_verified_acceptance",
  "verificationProofId",
  "δεν έγινε τοπική αλλαγή κατάστασης",
  "δεν άλλαξε τοπικά καμία εντολή",
]
for marker in required:
    if marker not in s: raise SystemExit('missing invariant: '+marker)
for forbidden in ["wo.status = 'Παραλήφθηκε'; // Auto-set when protocol accepted","wo.status='Παραλήφθηκε'; wo.completionDate=today"]:
    if forbidden in s: raise SystemExit('old local acceptance mutation remains: '+forbidden)
if s==orig: raise SystemExit('no client changes produced')
p.write_text(s,encoding='utf-8')
print('server-authoritative acceptance client patch applied')
